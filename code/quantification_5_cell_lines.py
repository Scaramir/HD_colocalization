...
def split_and_create_mask(img, file, save_mask = False):
    ...
    return ch1, ch2, ch3, mask_chchd2_and_tom20
...
def calculate_mean_intensity_of_2_markers(pic_folder_path, treatment_var="normal", threshold_mode="triangle_on_dapi_intensity_greater_1_on_rest", gaussian_filter=False, save_mask=False):
    ...
    for file in tqdm(glob.glob(pic_folder_path + "_thresholded_" + threshold_mode + "/*" + "*thresholded*.bmp"), desc="Counting pixels"):
        img = read_bmp(file)
        # Split the image into its three channels and create the colocalization mask:
        ch1, ch2, ch3, mask_chchd2_and_tom20_bin = split_and_create_mask(img, file, save_mask)
        ...
        # Delete the image from main memory
        del img
        gc.collect()
...
def box_plt_by_cell_line(quantification_df, value_to_plot, pic_folder_path, condition, threshold_mode, show="True"):
    ...
    plt.savefig(pic_folder_path + "_thresholded_" +
                threshold_mode + "/mannwhitneyplot_" + value_to_plot + "_" + condition + ".png", bbox_inches='tight')
    if show:
        plt.show()
    return
